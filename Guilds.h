...
void save_guilds() {
	//cout << "[SERVER] Autosave is saving guilds! Time of action: " << currentDateTime() << "." << endl;
	for (Guild to_save : guilds) {
			...
			//cout << "[SERVER] Guild [" << to_save.guild_name << "] is succesfully saved! Time of action: " << currentDateTime() << "." << endl;
					...
					if (search_member->display_name == "") {
						search_member->display_name = "`o" + search_member->member_name;
							...
							if (ifs.is_open()) {
								...
								if (j["dev"] == 1) search_member->display_name = "`6@" + search_member->member_name;
							}
					}
